...
This document contains a comprehensive prompt for an AI agent to build a complete Professional CMS Portfolio system from scratch using Eleventy, Netlify CMS, and automated blog deployment.
...
├── .github/
│   └── workflows/
│       └── sync-to-blog-repo.yml        # Syncs posts TO backup repo
├── eleventy.config.js
├── package.json
├── netlify.toml
├── site.config.json
...
- Single `blog/` folder as source of truth for all blog posts
- GitHub Actions automatically syncs posts to backup repository
- Netlify CMS only (no custom CMS)
- No git submodules - simpler deployment
- Feature flags via `site.config.json` for easy customization
```
...
1. **Collections**: 
   - `posts`: Read from `src/blog/` (single source of truth)
   - `allPosts`: All posts including drafts (for admin dashboard)
   - `projects`: From `src/projects/`
   - `postsByTag`: Group posts by tags
   - `allTags`: Extract unique tags

2. **Filters**:
   - `readableDate`: Format dates (MMMM DD, YYYY)
   - `shortDate`: Format dates (MMM DD, YYYY)
   - `excerpt`: Extract first paragraph or 200 characters
...
Blog listing:
- All published posts from `blog/` folder
...
**Create Single Blog Folder System**:

1. `www/src/blog/` - Main repository folder (single source of truth)
   - All blog posts stored here
   - Netlify CMS can edit these directly
   - Automatically synced to ll.db.blog for backup via GitHub Actions

**Why Single Folder System?**:
- Simpler architecture without git submodule complexity
- Netlify CMS works directly with main repository
- Automatic backup via GitHub Actions to separate repository
- Easier development and deployment

**Eleventy Config for Blog**:
```javascript
// Collections read from single source
eleventyConfig.addCollection("posts", function(collectionApi) {
  const allPosts = collectionApi.getFilteredByGlob("src/blog/**/*.md");
  return allPosts
    .filter(post => !post.data.draft && !isHidden(post.fileSlug))
...
8. ✅ Netlify CMS (/admin/cms/) loads and can edit blog posts
